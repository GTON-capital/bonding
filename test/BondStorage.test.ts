...
describe("BondStorage", function () {
    ...
    it("Check mints of user", async () => {
        ...
        await storage.mint(alice.address, 0, 0); // 0 id
        await storage.mint(alice.address, 0, 0); // 1 id
    ...
    it("Mint accessible to owner only", async() => {
        await expect(storage.connect(alice).mint(alice.address, 0, 0)).to.be.revertedWith("AdminAccess: restricted to admin or owner only");
        await storage.mint(alice.address, 0, 0);
