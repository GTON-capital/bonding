...
describe("OracleBondingETH", function () {
    ...
    let bonding: GTONOracleBondingETH;
    let sgton: MockStaking;
    let gton: MockERC20
    let token: MockERC20

    before(async () => {
        Bonding = await ethers.getContractFactory("GTONOracleBondingETH", wallet)
    ...
    async function deployDefaultBonding() {
        return await Bonding.deploy(
            ...
            gton.address,
            sgton.address,
            ethers.utils.formatBytes32String("7d")) as GTONOracleBondingETH;
    }
